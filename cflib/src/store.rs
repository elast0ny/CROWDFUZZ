...
/// (Box<&mut Vec<String>) List of arguments passed to the target
...
/* Corpus management */
/// (Box<&mut Vec<CfInputInfo>>) Current list of possible inputs
pub const STORE_INPUT_LIST : &str = "input_list";
/// (Box<&mut Vec<&mut CfInput>) New inputs that should be saved/tracked
pub const STORE_NEW_INPUTS : &str = "new_inputs";

/* Input selection */
/// (Box<&mut CfInput>) Index of the select input in INPUT_LIST
pub const STORE_INPUT_IDX : &str = "input_idx";
/// (Box<&mut CfInput>) Contents of the selected input
pub const STORE_INPUT_BYTES : &str = "input_bytes";

/* Input mutation */
/// (Box<&mut CfInput>) Contents of the mutated input
...
/// (Box<&mut TargetExitStatus>) The exit status for the last run
